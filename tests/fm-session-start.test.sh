...
EOF
  ...
  inventory_line=$(printf '%s\n' "$out" | grep -n '^--- Task · A (id: task-a) ---$' | head -1 | cut -d: -f1)
  ...
  [ -n "$missing_line" ] || fail "forced MISSING diagnostic did not appear at all"
...
EOF
  ...
  assert_contains "$out" "--- Compact · Startup (id: compact-startup) ---" \
    "in-flight display plus exact identity disappeared from startup recovery digest"
