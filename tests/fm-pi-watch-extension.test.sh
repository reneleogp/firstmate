...
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  # Leave enough startup budget that runner scheduling cannot kill a spawned retry before the fixture records it.
...
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  # Leave enough startup budget that runner scheduling cannot kill the unretired fixture before it records its arm.
...
if (denied !== "read-only") {
  console.error(`expected read-only, got ${denied}`);
  process.exit(1);
}
if (existsSync(process.env.FM_ARM_LOG)) {
