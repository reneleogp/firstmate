...
Stale session-lock recovery follows the holder verdict owned by [`bin/fm-session-lock-lib.sh`](../bin/fm-session-lock-lib.sh), so a recycled pid cannot block recovery while a live owner, absent lock, or malformed lock keeps the competing hook inert.
