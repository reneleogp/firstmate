...
A session-lock owner that fails the shared holder predicate - dead, no longer a harness, or a live process in a different kernel generation than the one that published the record, which is what a recycled pid looks like - is reclaimed through `bin/fm-lock.sh` before auto-arm state changes, while a live owner, absent lock, or malformed lock keeps the competing hook inert.
[`bin/fm-session-lock-lib.sh`](../bin/fm-session-lock-lib.sh) owns that record's format, its process-generation identity, and every verdict drawn from it.
