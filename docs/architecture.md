...
Actionable wakes include newly changed captain-relevant status, secondmate parent-directed status, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
Human-owned decisions, blockers, captain holds, failures, and review-ready or terminal results are edge-triggered by `bin/fm-human-notify-lib.sh`: one evidence fingerprint is recorded after durable publication, unchanged evidence is absorbed before model invocation, and resolution removes a keyed-wait receipt so a later condition can notify.
The fingerprint includes condition identity, meaningful evidence, and task incarnation but never elapsed time; a publication crash can duplicate one notice but cannot lose the first one.
Each recheck that finds an external wait's reason, status evidence, worker condition, and task incarnation unchanged doubles its next recheck window up to `FM_PAUSE_RESURFACE_MAX_SECS`, while any change in that evidence returns the wait to the base cadence.
When no unrelated actionable wake ends the poll first, all external-wait and worktree-write rechecks collected in that poll are delivered in a single wake naming each affected window, so sibling waits blocked on the same thing cost one turn rather than one turn each.
Only external-wait and worktree-write rechecks are batched or backed off; failures, changed human-owned evidence, check results, and stopped crews stay immediate.
...
That deferral re-surfaces on the same base and capped backoff cadence as a declared wait, with a reason naming the write evidence rather than a wedge, and it is bounded to one pruned, depth-bounded, wall-clock-bounded walk (`FM_WORKTREE_WRITE_PRUNE`, `FM_WORKTREE_WRITE_MAXDEPTH`, `FM_WORKTREE_WRITE_TIMEOUT`) taken only in the branch that was about to escalate, never on every poll.
...
A crew that declared an external wait (`paused:`) or a verified captain-held transfer is the exception to that bound: the external wait takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck, while the human-owned transfer remains silent until its evidence changes or it is answered.
Lifting either declaration restores the unchanged busy-pane wedge path, while a pane that is no longer busy returns to its matching external-wait or human-owned-wait classification.
...
No-verb transport notifications, such as `working:` notes and bare turn-ended signals, are absorbed before model invocation without treating either the notification or an old status line as current state.
The independent stale-pane path still detects a worker that stopped and consults `bin/fm-crew-state.sh` for positive current-state evidence before suppressing that condition.
A `kind=secondmate` task's changed status is the parent-directed reply stream and remains actionable; its bare turn-ended signal is absorbed like every other no-status completion notification.
A crew that declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced only on the longer external-wait cadence, rather than being treated as a possible wedge.
A verified `captain-held` transfer is also exempt from wedge detection, but it has no timer: the durable backlog and on-demand views retain it until the captain answers, defers, or releases it.
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies the external cadence to an unchanged `paused:` endpoint only when the backend confidently reports its agent dead.
Live or inconclusive liveness remains fail-open at that initial surface, and a secondmate's endpoint liveness is still never read at all; a mate enters the external cadence only after declaring `paused:`.
The initial routine status signal remains silent in normal mode, while away mode self-handles it.
...
A declared external wait trades that silence for a bounded recheck because its dependency may clear without a human; captain-held work stays visible in recovery and explicit status surfaces without creating timed turns.
...
Pi and OpenCode verify session and delivery ownership before persistent-adapter follow-ups, while Pi transfers sole watcher ownership to the away daemon and restores one ordinary cycle on return.
[`watcher-continuity.md`](watcher-continuity.md) owns the successor, late-callback, intentional-retirement, standdown, and return contracts.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs and status-scan primitives, and share `bin/fm-human-notify-lib.sh` for human-owned notification identity, evidence, resolution, and presentation.
...
The always-on watcher uses that library's captain-relevance classification for changed status and its current-state absorb classification on first-sighting stale panes before status-log terminality is trusted, while the daemon maintains distinct wedge and declared-wait recheck cadences.
...
The daemon escalates changed captain-relevant events plus bounded external-pause rechecks as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh`; unchanged human-owned conditions are absorbed before injection.
...
Task metadata also separates immutable task identity from one presentation-only `display_name`; [Task display names](configuration.md#task-display-names) owns the cross-backend contract and applicability table.
