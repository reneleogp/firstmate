...
Actionable wakes include newly changed captain-relevant status, secondmate parent-directed status, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
Each recheck that finds a declared wait's reason, status evidence, worker condition, and task incarnation unchanged doubles its next recheck window up to `FM_PAUSE_RESURFACE_MAX_SECS`, while any change in that evidence returns the wait to the base cadence.
When no unrelated actionable wake ends the poll first, all declared-wait and worktree-write rechecks collected in that poll are delivered in a single wake naming each affected window, so sibling waits blocked on the same thing cost one turn rather than one turn each.
Only declared-wait and worktree-write rechecks are batched or backed off; failures, decisions, check results, and stopped crews stay immediate.
...
Its initial routine status signal remains silent in normal mode, while away mode self-handles it and owns the later recheck.
...
Pi additionally revalidates the exact recovery generation, successor watcher, and still-pending queue reason immediately before scheduling a follow-up, so an acknowledged late close is absorbed rather than spending a model turn.
While `state/.afk` exists, the Pi extension uses the arm layer's authenticated intentional-retirement path to stop its exact watcher without publishing a false recovery check, then publishes a generation-bound standdown receipt before the away launcher may start the daemon.
The daemon marks its own exact watcher retirement on return before the launcher clears `.afk`, after which the extension restores one ordinary arm when that session previously owned monitoring; every unmarked watcher close retains ordinary recovery and `check: rearm-resurface` delivery.
