...
Each recheck that finds a declared wait's evidence unchanged doubles its next recheck window up to `FM_PAUSE_RESURFACE_MAX_SECS`, any change in that evidence returns the wait to the base cadence, and every wait that comes due in one poll is delivered as a single wake naming each of them, so waits blocked on the same thing cost one turn rather than one turn each.
Only the declared-wait recheck is batched or backed off; failures, decisions, check results, and stopped crews stay immediate.
