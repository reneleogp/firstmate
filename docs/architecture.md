...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A crew that declared an external wait (`paused:`) is the one exception to that bound: its busy verdict supplies liveness while identifying the long-running foreground call as the declared wait, so it takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation.
...
A verified `captain-held` transfer is a human-owned wait, so it is absorbed without a timed reminder until its evidence changes or the captain answers.
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake for an unchanged `paused:` endpoint, then applies that same cadence; the pause classification itself is recovered only when the backend confidently reports its agent dead.
...
A paused endpoint's later sights are still held to that same bounded cadence rather than re-alarming on every pane-hash change, because the throttle is keyed to the declaration and not to the pane an idle parked worker keeps ticking.
A secondmate's endpoint liveness is still never read at all; a mate is admitted to the declared-wait cadence only to serve a paused wait's bounded re-surface.
...
A declared external wait trades that silence for one bounded recheck per pause window, naming which human the wait is on, while a verified captain-held transfer remains silent until its evidence changes or the captain answers.
...
The daemon escalates captain-relevant events, plus a bounded recheck for a declared pause that is still declared, naming which human that wait is on, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
