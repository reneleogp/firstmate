...
The macOS semantic regression uses a fake launchctl and filesystem, validates the generated plist through plistlib and plutil when the runner supplies it, rejects foreign ownership and malformed files, checks XML escaping and secret absence, exercises bootstrap and bootout races, verifies active and inactive lifecycle states, and proves secure temporary-audio cleanup, crash reconciliation, FFmpeg conversion, uninstall, and no Telegram credentials.
