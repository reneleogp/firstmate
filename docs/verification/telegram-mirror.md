...
ok - Telegram source replies, fallback, voice revisions, copy_text, ForceReply, callback replay, and state transitions
```
...
The native reply regression uses an executable fake Bot API and proves exact source targets for text, voice, queued, transcribing, transcript, Pi delivery, and Firstmate response messages, deleted-target fallback, outbound-ID journaling, queued voice ordering, edit/copy_text/ForceReply transitions, retry and failure recovery, stale revisions, duplicate callback acknowledgement, and mode-off refusal.
