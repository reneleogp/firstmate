...
7. After an actionable child close, the extension rechecks session-lock ownership, verifies one successor, and confirms the exact wake reason is still queued before it delivers the follow-up; an acknowledged late close remains silent and its bounded fallback is defined in `docs/watcher-continuity.md`.
8. While away mode is active, the extension retires its ordinary child and the away daemon is the only delivery owner; return restores one ordinary child automatically when this generation previously owned monitoring.
9. Ordinary work, turn completion, and ordinary signal, stale, check, heartbeat, or other wake handling: do not call `fm_watch_arm_pi` again because continuity is extension-owned rather than model-memory-owned.
10. An unexpected child close enters bounded exponential retry, and an exhausted retry or lost session lock is surfaced as a watcher failure instead of disappearing.
11. Missing, failed, or unhealthy cycle only: if a later notification explicitly reports one of those repair conditions, drain queued wakes, inspect the failure text, call `fm_watch_arm_pi`, and restart the selected Pi-family executable with both extensions loaded if needed.
