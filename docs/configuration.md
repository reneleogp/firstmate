...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; a declared external wait takes the FM_PAUSE_RESURFACE_SECS recheck below instead, while a verified captain-held transfer remains a silent human-owned wait
FM_PAUSE_RESURFACE_SECS=3600       # seconds between bounded rechecks of a declared external wait, including a live idle pane after its first inconclusive stale wake and a live busy pane past FM_BUSY_TURN_MAX_SECS; verified captain-held transfers are silent until their evidence changes or the captain answers; the away-mode daemon uses the same setting for declared external waits, ageing its window against the crew's own latest status line rather than pane busy state
