...
FM_PAUSE_RESURFACE_MAX_SECS=43200  # cap on that recheck window; each recheck that finds the wait's reason, status line, or task incarnation unchanged doubles the next window up to this cap (1h, 2h, 4h, 8h, then 12h by default), any change returns the wait to the base cadence above, and waits that come due in the same pass are delivered as one notification naming each of them; both supervision paths read this setting
