...
- The extension mirrors only from the session that holds the Firstmate home's session lock, checked against the running process's own ancestry and against the kernel process generation recorded when that lock was published, so a reused process id cannot inherit someone else's mirror.
