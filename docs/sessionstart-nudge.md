...
Before printing, the nudge wrapper reads only the pid on the first line of `state/.lock` and walks at most eight parents from its own pid in a deliberately loose, hard-coded loop, while `bin/fm-session-lock-lib.sh` owns the complete record format and authoritative ownership verdict.
If that pid is live in the wrapper's ancestry, session start already ran in this harness session and the wrapper stays silent; Pi's primary extensions instead obtain their complete ownership verdict through `bin/fm-lock.sh`.
