...
An ordinary `fm-send.sh` text steer no longer rides this verified submit at all: it becomes a durable steering-inbox record plus a best-effort constant doorbell line (`bin/fm-task-inbox-lib.sh`), so the verdicts above are delivery-critical only for the typed plane - harness-native invocations, explicit backend targets, and the remote leg - where `fm-send.sh` still never retypes or assumes a confirmed submit for an unconfirmed verdict; its header owns the distinct delivered-unconfirmed exit status and operator response.
