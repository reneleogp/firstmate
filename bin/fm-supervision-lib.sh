...
# (state/x-watch.check.sh), or an enabled Telegram transport, and whether its watcher has a fresh liveness beacon
...
#   FM_SUP_TELEGRAM       true/false - home-scoped Telegram supervision is enabled
#   FM_SUP_NEEDED         true/false - in-flight work, an X-mode relay poll, an
#                         enabled Telegram transport, or a registered event source
...
fm_supervision_status() {
  ...
  [ -f "$state/telegram/enabled" ] && FM_SUP_TELEGRAM=true
  if [ "$FM_SUP_IN_FLIGHT" -gt 0 ] \
    || [ -f "$state/x-watch.check.sh" ] \
    || [ "$FM_SUP_TELEGRAM" = true ] \
