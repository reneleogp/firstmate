...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  if [ "$DISPLAY_NAME_SET" -eq 0 ]; then
    DISPLAY_NAME=$(fm_meta_get "$RELAUNCH_META" display_name)
    [ -n "$DISPLAY_NAME" ] || DISPLAY_NAME=$(fm_display_name_fallback "$ID")
  fi
