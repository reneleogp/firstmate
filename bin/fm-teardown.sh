...
# destructive sequence below (worker reap, worktree return, pane close, record
# removal) runs under the named-session presentation lock, acquired BEFORE
# anything is reaped, returned, or erased. A contended or unresolvable lock
# refuses while the isolated copy, durable records, and endpoint are intact.
...
# Correlate a projected endpoint while its metadata, process, and live Herdr
# shape are still intact. Reaping a restored shell can remove the presentation
# projection before pane cleanup, but must not quarantine an endpoint already
# proven exact under the session lock.
...
# --force, and before any remaining destructive step - a still-parked run or a
...
fm_human_notify_clear_task "$STATE" "$ID" || exit 1
