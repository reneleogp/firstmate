...
- `telegram-respond` - load on a `check: telegram <local-request-id>` wake, and on a decision, blocker, terminal-confirmation, PR-ready, or terminal lifecycle wake carrying `<work-id>` only when `bin/fm-telegram.py active-request --work-id <work-id>` succeeds, to route only the required replies to the durably bound pinned chat.
